import React from "react";
import { useNavigate } from 'react-router-dom'
import { Dropdown } from 'flowbite-react'
import 'animate.css';
import data from './../../utils/json/data.json'
import logo from './../../assets/images/logo.png'

function Aboutus (props){
  const navigate = useNavigate();
  const onClickPrev = () => {
    navigate('/account', {state: {CurrentPage: 3}})
  }
  const leaderboards = data.leaderboard
  return (
    <>  
      <div className="page justify-between bg-black">
        <div className="header">
          <div className="flex">
            <button className="rounded-full min-w-[50px] mt-[10px] ml-0 p-[5px] bg-transparent hover:outline-none focus:bg-none focus:outline-none hover:bg-[#1d1d1d]" onClick={onClickPrev}><p className=" focus:border-none zmdi zmdi-chevron-left text-white text-4xl"></p></button>
          </div>
          <div className="flex">
            <img src={logo} className="h-32 m-auto mt-[47px] animate__animated animate__zoomIn wow" data-wow-duration="0.3s" />
          </div>
        </div>
        <div>
          <div className="absolute left-0 right-0 bottom-0 w-screen back_content rounded-t-3xl h-[calc(100vh-325px)] bg-[#151d24] animate__animated animate__zoomInUp wow" data-wow-duration="0.8s">
            <div className="w-full p-6 pt-7">
              <p className="text-left text-white text-lg pb-4">
                Who we are?
              </p>
              <p className="text-left text-white text-md pb-4">
              Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
              </p>
              <p className="text-left text-white text-lg pb-4">
              How we started?
              </p>
              <p className="text-left text-white text-md pb-4">
              Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
              </p>
              <p className="text-left text-white text-lg pb-4">
              Who we are?
              </p>
              <p className="text-left text-white text-md pb-4">
              Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
              </p>
              <p className="text-left text-white text-lg pb-4">
              How we started?
              </p>
              <p className="text-left text-white text-md pb-4">
              Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
              </p>
            </div>
            
          </div>
        </div>
      </div>  
    </>
  )
}
export default Aboutus;